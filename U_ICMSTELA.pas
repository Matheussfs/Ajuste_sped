...
type
  TFRM_TELA_ICMS = class(TForm)
    ...
    procedure Btn_calcular_icmsKeyPress(Sender: TObject; var Key: Char);
    procedure Btn_limpar_icmsKeyPress(Sender: TObject; var Key: Char);
  private
...
procedure TFRM_TELA_ICMS.Btn_calcular_icmsKeyPress(Sender: TObject;
  var Key: Char);
begin
begin
if key = #13 then
...
//configurando o botão de limpar, preenchendo vazio em tudo após o clique
...
procedure TFRM_TELA_ICMS.Btn_limpar_icmsKeyPress(Sender: TObject;
  var Key: Char);
begin
begin
if key = #13 then
...
{proteção para não deixar o tedit de base de icms vazio,
...
{proteção para não deixar preencher letras no tedit.
